...
@section('content')
    ...
    <h3 class="header-title2">Food</h3>
...
<div class="posts-box" >
        ...
        @endif
            {{-- <td> --}}
                @if($task->user_id == Auth::id())
                    <form action="{{ route('food.destroy',['id'=>$task->id]) }}" method="POST">
                        @csrf
                        @method('delete')
                        <input type="hidden" name="task_id" value="{{ $task->id }}">
    ...
    <div class="yahho">
        ...
        <form action="{{ route('comment.food', ['task_id' => $task->id]) }}" method="POST" class="commentForm">
        @csrf
    ...
    <div class="pagination-links">
        ...
        <div class="create-post-section">
            ...
            <div class="create-post">
                <form class="d-flex justify-content-evenly" method="POST" action="{{ route('food.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="parent">
                        <div class="mb-3">
                            <p>Image</p>
                    ...
                    <div class="parent">
                        <input type="number" name="category_id" value="3" hidden>
...
@push('js')
    ...
    if (confirm('タスクを削除してもよろしいですか？')) {
        ...
        console.log(taskId)
        $.ajax({
            url: '/food/' + taskId, // タスクのIDに応じてURLを設定
            type: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
            },
